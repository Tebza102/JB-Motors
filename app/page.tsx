...
import { ProblemReportForm } from "./features/problem-reports/ProblemReportForm";
import { clearProblemReports, loadProblemReports, saveProblemReports } from "./features/problem-reports/storage";
import { getReportUrgency, ProblemReport, ProblemReportStatus, ReportMutationResult, REPORT_STATUSES } from "./features/problem-reports/types";
import { ESTIMATE_DOCUMENT, INVOICE_DOCUMENT, MONTHLY_REPORT_DOCUMENT, PdfDocumentKind, SERVICE_HISTORY_DOCUMENT } from "./features/documents/data";
import { downloadPdf } from "./features/documents/pdf";
...
type InterfaceState = "ready" | "loading" | "empty" | "error";
...
const WORKSHOP_PROFILE: WorkshopProfile = {
  businessName: "JB Motors",
  ownerName: "Jabulani",
  ownerRole: "Owner / Administrator",
  location: "Ratanda, Heidelberg, Gauteng (GP)",
  logoPath: "/brand/jb-motors-logo.png",
  contact: { telephone: null, whatsapp: null, email: null, streetAddress: null, vatNumber: null, labourRate: null },
};
...
function BrandLockup({ compact = false, dark = true }: { compact?: boolean; dark?: boolean }) {
  return (
    <div className={`brand-lockup ${compact ? "compact" : ""} ${dark ? "on-dark" : "on-light"}`} aria-label={`${WORKSHOP_PROFILE.businessName} logo`}>
      <Image className="brand-logo-image" src={WORKSHOP_PROFILE.logoPath} alt="JB Motors" width={510} height={184} priority />
    </div>
  );
}

function InterfaceStatePanel({ state, onReset }: { state: Exclude<InterfaceState, "ready">; onReset: () => void }) {
  const content = {
    loading: ["Preparing workshop view", "Demo records are being organised for this screen."],
    empty: ["No records in this view", "Adjust the filters or return to the populated demo state."],
    error: ["This demo view could not load", "No workshop data was changed. Retry the local interface."],
  }[state];
  return <section className={`panel interface-state state-${state}`} aria-live="polite"><span>{state === "loading" ? "…" : state === "empty" ? "0" : "!"}</span><h2>{content[0]}</h2><p>{content[1]}</p><button className="primary-button" onClick={onReset}>{state === "error" ? "Retry" : "Return to demo data"}</button></section>;
}
...
function PdfDownloadButton({
  ...
  const handleDownload = async () => {
    ...
    setError("");
    try {
      const filename = await downloadPdf(kind);
      setState("idle");
      notify?.(`${filename} download started`);
    } catch (reason) {
      setState("error");
      setError(reason instanceof Error ? reason.message : "The PDF could not be generated. Try again.");
    }
  };
  return <span className="download-action">
    <button type="button" className={className} onClick={handleDownload} disabled={state === "working"}>{state === "working" ? "Generating PDF…" : label}</button>
...
function Dashboard({ onOpen, notify }: { onOpen: (view: WorkshopView) => void; notify: (message: string) => void }) {
  ...
  return (
    <>
      <PageHeader eyebrow="WORKSHOP DASHBOARD" title="What needs your attention today?" description="A live operational view of jobs, customers and workshop follow-ups." actions={<div className="open-status"><span /> Local prototype <strong>Demo data</strong></div>} />
...
function VehicleProfile({ notify }: { notify: (message: string) => void }) {
  ...
  return (
    <>
      ...
      {tab === "Kilometres" ? <KilometreHistory /> : tab === "Service History" ? <ServiceHistory notify={notify} /> : (
...
function ServiceHistory({ notify }: { notify: (message: string) => void }) {
  return <section className="panel service-history"><div className="panel-heading"><div><span className="section-kicker">COMPLETE VEHICLE RECORD</span><h2>Service history</h2></div><PdfDownloadButton kind="service-history" label="Download history PDF" notify={notify} /></div>{SERVICE_HISTORY_DOCUMENT.entries.map((entry) => <div className="service-record" key={entry.date}><time>{entry.date}</time><strong>{entry.service}</strong><span>{entry.kilometres}</span><span>{entry.jobReference}</span><b>{entry.total}</b><button>Open record →</button></div>)}</section>;
}

function JobCardDetail({ notify }: { notify: (message: string) => void }) {
  const [view, setView] = useState<"detail" | "board" | "table">("detail");
  const [status, setStatus] = useState("In Progress");
  if (view === "board") return <JobBoard onOpen={() => setView("detail")} />;
  if (view === "table") return <JobTable onOpen={() => setView("detail")} onBoard={() => setView("board")} />;
  return (
    <>
      <PageHeader eyebrow="JOB CARDS / OPERATIONAL WORK" title="JB-JOB-0248" description="Ford Ranger 2.2 TDCi · NW 81 LM GP · Sipho Dlamini" actions={<><button className="secondary-button" onClick={() => setView("table")}>Table view</button><button className="secondary-button" onClick={() => setView("board")}>Board view</button><button className="secondary-button">Print job card</button><button className="primary-button" onClick={() => notify("Job card changes saved")}>Save changes</button></>} />
      <section className="status-control panel"><div><span className="section-kicker">JOB STATUS</span><strong>{status}</strong><small>Last changed today at 09:42 by Jabulani</small></div><div className="status-steps">{["Scheduled","In Progress","Waiting Parts","QC","Ready"].map((step) => <button className={status === step ? "active" : ""} key={step} onClick={() => {setStatus(step);notify(`Job moved to ${step}`);}}><span />{step}</button>)}</div><button className="compact-button">More actions</button></section>
...
function JobTable({ onOpen, onBoard }: { onOpen: () => void; onBoard: () => void }) {
  const jobs = [
    ["JB-JOB-0248", "Sipho Dlamini", "Ford Ranger 2.2 TDCi", "NW 81 LM GP", "Petrus Nkosi", "In Progress", "Today 16:00"],
    ["JB-JOB-0249", "Thabo Mokoena", "Toyota Corolla 1.8", "GP 123-456", "Petrus Nkosi", "Scheduled", "Today 14:30"],
    ["JB-JOB-0246", "Naledi Khumalo", "Hyundai i20 1.4", "GP 882-410", "Mandla Dube", "Waiting Parts", "2 days"],
    ["JB-JOB-0244", "Lerato Molefe", "VW Polo 1.0 TSI", "CA 459-882", "Mandla Dube", "QC", "Today 12:15"],
    ["JB-JOB-0242", "Kabelo Ndlovu", "Isuzu D-Max", "MP 28 TK GP", "Petrus Nkosi", "Ready", "Today 10:40"],
  ];
  return <><PageHeader eyebrow="JOB CARDS / TABLE VIEW" title="Workshop job cards" description="A compact operational list for scanning workload, ownership and due times." actions={<><button className="secondary-button" onClick={onBoard}>Board view</button><button className="primary-button" onClick={onOpen}>Open JB-JOB-0248</button></>} /><section className="panel"><div className="filter-bar"><label className="search-field"><span>⌕</span><input placeholder="Search job, customer, registration…" /></label><div className="filter-chips"><button className="active">Open jobs</button><button>Waiting parts</button><button>QC</button><button>Ready</button></div></div><div className="responsive-table generic-table"><div className="table-head" style={{gridTemplateColumns:"1fr 1.1fr 1.5fr .9fr 1fr .9fr .8fr"}}>{["Job","Customer","Vehicle","Registration","Technician","Status","Due"].map((header) => <span key={header}>{header}</span>)}</div>{jobs.map((row) => <button className="table-row job-table-row" style={{gridTemplateColumns:"1fr 1.1fr 1.5fr .9fr 1fr .9fr .8fr"}} key={row[0]} onClick={onOpen}>{row.map((cell,index) => <span data-label={["Job","Customer","Vehicle","Registration","Technician","Status","Due"][index]} key={cell}>{index===5 ? <StatusPill tone={cell==="Ready"?"green":cell==="Waiting Parts"?"amber":cell==="QC"?"blue":"grey"}>{cell}</StatusPill> : cell}</span>)}</button>)}</div></section></>;
}
...
function EstimateDetail({ onConvert, notify }: { onConvert: () => void; notify: (message: string) => void }) {
  ...
  return <><PageHeader eyebrow={`ESTIMATES / ${doc.reference}`} title="Estimate detail" description={`Prepared for ${doc.customer.name} · ${doc.vehicle.label} · ${doc.vehicle.registration}`} actions={<><button className="secondary-button" onClick={() => notify("Estimate preview opened")}>Preview</button><PdfDownloadButton kind="estimate" label="Download PDF" notify={notify} /><button className="primary-button" onClick={onConvert}>Convert to job card</button></>} /><div className="document-layout"><section className="panel document-sheet"><div className="document-brand"><BrandLockup dark={false} /><div><StatusPill tone="green">{doc.status}</StatusPill><h2>{doc.type}</h2><span>{doc.reference}</span></div></div><div className="document-parties"><div><span>PREPARED FOR</span><strong>{doc.customer.name}</strong><p>{doc.customer.phone}<br />{doc.customer.email}</p></div><div><span>VEHICLE</span><strong>{doc.vehicle.label}</strong><p>{doc.vehicle.registration}<br />{doc.vehicle.kilometres}</p></div><div><span>DETAILS</span><strong>Created {doc.created}</strong><p>Valid until {doc.validUntil}<br />Prepared by {doc.preparedBy}</p></div></div><div className="document-lines"><div className="doc-line-head"><span>Description</span><span>Qty</span><span>Unit price</span><span>Total</span></div>{doc.lines.map((line) => <div key={line.description}><span><strong>{line.description}</strong><small>{line.detail}</small></span><span>{line.quantity}</span><span>{line.unitPrice}</span><strong>{line.total}</strong></div>)}</div><div className="document-bottom"><div className="document-notes"><span>NOTES</span><p>{doc.notes}</p><span>VALIDITY</span><p>{doc.terms}</p></div><div className="document-totals">{doc.totals.map(([label, value], index) => <div className={index === doc.totals.length - 1 ? "grand-total" : ""} key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></div></section><aside className="document-aside"><section className="panel approval-card"><span className="section-kicker">APPROVAL STATUS</span><StatusPill tone="green">APPROVED</StatusPill><h3>Approved by customer</h3><p>{doc.customer.name} approved this estimate by WhatsApp confirmation.</p><dl><div><dt>Date</dt><dd>20 Aug 2026</dd></div><div><dt>Time</dt><dd>08:46</dd></div><div><dt>Recorded by</dt><dd>{doc.preparedBy}</dd></div></dl><button className="wide-button">View approval evidence</button></section><section className="panel"><div className="panel-heading"><div><span className="section-kicker">ACTIVITY</span><h2>Estimate timeline</h2></div></div><div className="timeline compact-timeline"><div><time>Today 08:46</time><strong>Estimate approved</strong><p>Customer confirmation recorded</p></div><div><time>Yesterday 15:14</time><strong>Estimate shared</strong><p>Sent by WhatsApp</p></div><div><time>Yesterday 14:52</time><strong>Estimate created</strong><p>{doc.preparedBy}</p></div></div></section></aside></div></>;
}

function InvoiceDetail({ notify }: { notify: (message: string) => void }) {
  const doc = INVOICE_DOCUMENT;
  return <><PageHeader eyebrow={`INVOICES / ${doc.reference}`} title="Invoice detail" description={`Issued to ${doc.customer.name} · Linked to ${doc.jobReference}`} actions={<><PdfDownloadButton kind="invoice" label="Download PDF" notify={notify} /><button className="secondary-button" onClick={() => notify("Invoice share link copied")}>Share</button><button className="primary-button" onClick={() => notify(`Payment capture opened for ${doc.reference}`)}>Record payment</button></>} /><div className="document-layout"><section className="panel document-sheet"><div className="document-brand"><BrandLockup dark={false} /><div><StatusPill tone="amber">{doc.status}</StatusPill><h2>{doc.type}</h2><span>{doc.reference}</span></div></div><div className="document-parties"><div><span>BILL TO</span><strong>{doc.customer.name}</strong><p>{doc.customer.phone}<br />{doc.customer.vatStatus}</p></div><div><span>VEHICLE</span><strong>{doc.vehicle.label}</strong><p>{doc.vehicle.registration}<br />{doc.vehicle.kilometres}</p></div><div><span>INVOICE DETAILS</span><strong>{doc.issued}</strong><p>Job: {doc.jobReference}<br />{doc.paymentTerms}</p></div></div><div className="document-lines"><div className="doc-line-head"><span>Description</span><span>Qty</span><span>Unit price</span><span>Total</span></div>{doc.lines.map((line) => <div key={line.description}><span><strong>{line.description}</strong><small>{line.detail}</small></span><span>{line.quantity}</span><span>{line.unitPrice}</span><strong>{line.total}</strong></div>)}</div><div className="document-bottom"><div className="payment-history"><span>PAYMENTS RECEIVED</span><div><strong>R2,500.00</strong><p>{doc.payment}</p></div><small>JB Motors VAT registration requires owner confirmation.</small></div><div className="document-totals">{doc.totals.map(([label, value], index) => <div className={index === doc.totals.length - 1 ? "grand-total" : ""} key={label}><span>{label}</span><strong>{label === "Paid" ? `−${value}` : value}</strong></div>)}</div></div></section><aside className="document-aside"><section className="panel balance-card"><span className="section-kicker">PAYMENT POSITION</span><strong>R3,606.50</strong><p>Outstanding balance due on vehicle collection.</p><div className="payment-progress"><span style={{width:"41%"}} /></div><small>41% paid</small><button className="primary-button wide-primary" onClick={() => notify("Payment capture opened")}>Record payment</button></section><section className="panel"><div className="panel-heading"><div><span className="section-kicker">PAYMENT METHODS</span><h2>Accepted</h2></div></div><div className="method-list"><span>EFT</span><span>Card</span><span>Cash</span></div></section></aside></div></>;
}
...
function Requests({ reports, storageWarning, onUpdateStatus, onReset, notify }: { reports: ProblemReport[]; storageWarning: string | null; onUpdateStatus: (id: string, status: ProblemReportStatus) => ReportMutationResult; onReset: () => void; notify: (message: string) => void }) {
  ...
  const urgentReports = openReports.filter((report) => ["High", "Vehicle cannot drive"].includes(getReportUrgency(report)));
  const changeStatus = (report: ProblemReport, status: ProblemReportStatus) => {
    const result = onUpdateStatus(report.id, status);
    if (result.ok) notify(`${report.reference} moved to ${status}`);
    else notify(result.message);
  };
  return <><PageHeader eyebrow="CUSTOMER PORTAL INBOX" title="Customer Requests" description="Review customer submissions, then contact them to confirm the next step." actions={<>{(reports.length > 0 || storageWarning) && <button className="secondary-button" onClick={onReset}>Reset demo reports</button>}<button className="primary-button">+ Record phone request</button></>} />{storageWarning && <div className="storage-warning" role="alert"><strong>Saved reports unavailable</strong><span>{storageWarning}</span></div>}<section className="metrics-grid metrics-four"><MetricCard value={String(2 + openReports.length).padStart(2, "0")} label="New requests" note={`${openReports.length} submitted from this browser`} tone="blue" /><MetricCard value={String(1 + openReports.length).padStart(2, "0")} label="Problem reports" note={`${urgentReports.length + 1} high severity`} tone="red" /><MetricCard value={String(4 + openReports.length).padStart(2, "0")} label="Awaiting contact" note="Includes saved demo reports" tone="amber" /><MetricCard value="09" label="Booked this week" note="From portal requests" tone="green" /></section><section className="request-grid">{reports.map((report) => {
    const urgency = getReportUrgency(report);
    const urgent = urgency === "High" || urgency === "Vehicle cannot drive";
    const expanded = expandedReportId === report.id;
    return <article className={`panel request-card ${urgent ? "urgent" : ""}`} key={report.id}><div className="request-card-top"><StatusPill tone={report.status === "Resolved" ? "green" : urgent ? "red" : "blue"}>{report.status.toUpperCase()}</StatusPill><time>{new Date(report.submittedAt).toLocaleString("en-ZA", { dateStyle: "medium", timeStyle: "short" })}</time></div><h2>{report.issues.length} vehicle {report.issues.length === 1 ? "issue" : "issues"}</h2><p className="request-vehicle">{report.vehicle.label} · {report.vehicle.registration}</p><div className="request-customer"><span className="avatar">{report.customer.name.split(" ").map((part) => part[0]).join("")}</span><div><strong>{report.customer.name}</strong><small>{report.customer.mobile} · {report.reference}</small></div></div><dl><div><dt>Issues</dt><dd>{report.issues.length}</dd></div><div><dt>Urgency</dt><dd><StatusPill tone={urgent ? "red" : urgency === "Medium" ? "amber" : "green"}>{urgency}</StatusPill></dd></div><div><dt>Status</dt><dd>{report.status}</dd></div><div><dt>Media</dt><dd>Not available</dd></div></dl>{expanded && <div className="reported-issues">{report.issues.map((issue, index) => <section key={issue.id}><span>ISSUE {index + 1}</span><h3>{issue.categories.join(" / ")}</h3><p>{issue.description}</p><dl><div><dt>Occurs</dt><dd>{issue.occurrence}</dd></div><div><dt>Severity</dt><dd>{issue.severity}</dd></div></dl></section>)}</div>}<div className="report-review-controls"><label><span>Review status</span><select value={report.status} onChange={(event) => changeStatus(report, event.target.value as ProblemReportStatus)}>{REPORT_STATUSES.map((status) => <option key={status}>{status}</option>)}</select></label><button className="secondary-button" onClick={() => setExpandedReportId(expanded ? null : report.id)}>{expanded ? "Hide issues" : "Review all issues"}</button><button className="secondary-button" onClick={() => notify(`Ready to call ${report.customer.name} on ${report.customer.mobile}`)}>Call customer</button></div></article>;
...
function Reports({ notify }: { notify: (message: string) => void }) {
  ...
  return <><PageHeader eyebrow="REPORTING / OPERATIONAL DECISIONS" title="Workshop Reports" description="Use performance, customer, quality and stock signals to decide what to do next." actions={<><button className="secondary-button" disabled title="Excel export is planned for a later phase">Export Excel · planned</button><PdfDownloadButton kind="monthly-report" label="Download monthly report" className="primary-button" notify={notify} /></>} /><div className="report-grid">{doc.groups.map((group) => <section className="panel report-card" key={group.title}><div className="panel-heading"><div><span className="section-kicker">{doc.period.toUpperCase()}</span><h2>{group.title}</h2></div><button className="text-button" onClick={() => notify(`${group.title} details are already shown in this prototype view`)}>Open report →</button></div><div className="report-stats">{group.stats.map(([value,label],index) => <div key={label}><strong>{value}</strong><span>{label}</span><i><b style={{width:`${group.bars[index]}%`}} /></i></div>)}</div><div className="report-insight"><StatusPill tone={group.title === "Workshop quality" ? "amber" : "green"}>INSIGHT</StatusPill><p>{group.insight}</p></div></section>)}</div></>;
}

function Settings({ notify }: { notify: (message: string) => void }) {
  const [section, setSection] = useState("Workshop profile");
  const sections = ["Workshop profile", "Workshop rules", "Numbering", "Service catalogue", "Payment methods", "Staff & roles", "Notifications"];
  const pending = "Requires owner confirmation";
  const content: Record<string, React.ReactNode> = {
    "Workshop profile": <><div className="panel-heading"><div><span className="section-kicker">BUSINESS IDENTITY</span><h2>Workshop profile</h2></div></div><div className="logo-setting"><BrandLockup dark={false} /><div><strong>Official JB Motors logo</strong><p>The supplied logo is used unchanged across the platform and workshop documents.</p><StatusPill tone="green">OFFICIAL ASSET</StatusPill></div></div><div className="form-grid"><label><span>Workshop name</span><input value={WORKSHOP_PROFILE.businessName} readOnly /></label><label><span>Owner / Administrator</span><input value={WORKSHOP_PROFILE.ownerName} readOnly /></label><label className="full"><span>Confirmed operating area</span><input value={WORKSHOP_PROFILE.location} readOnly /></label>{[["Telephone",WORKSHOP_PROFILE.contact.telephone],["WhatsApp",WORKSHOP_PROFILE.contact.whatsapp],["Email",WORKSHOP_PROFILE.contact.email],["Street address",WORKSHOP_PROFILE.contact.streetAddress],["VAT number",WORKSHOP_PROFILE.contact.vatNumber],["Labour rate",WORKSHOP_PROFILE.contact.labourRate]].map(([label,value]) => <label key={label}><span>{label}</span><input value={value ?? pending} readOnly className={!value ? "pending-input" : ""} /></label>)}</div></>,
    "Workshop rules": <SettingsCards kicker="OWNER CONFIRMATION" title="Workshop rules" items={[["Default service interval",pending],["Service reminder threshold",pending],["Labour rate",pending],["VAT treatment",pending],["Estimate validity",pending]]} />,
    "Numbering": <SettingsCards kicker="DOCUMENT CONTROL" title="Numbering" items={[["Estimate","JB-EST-0001"],["Job card","JB-JOB-0001"],["Invoice","JB-INV-0001"]]} />,
    "Service catalogue": <SettingsCards kicker="SERVICE CATALOGUE" title="Available service categories" items={[["Routine service","Active"],["Vehicle inspection","Active"],["Mechanical repair","Active"],["Electrical fault","Active"],["Diagnostic check","Active"]]} />,
    "Payment methods": <SettingsCards kicker="PAYMENT METHODS" title="Workshop payment options" items={[["Cash","Enabled for prototype"],["EFT","Enabled for prototype"],["Card","Enabled for prototype"]]} />,
    "Staff & roles": <SettingsCards kicker="ROLE ACCESS" title="Staff and permissions" items={[["Jabulani","Owner / Administrator"],["Service advisor","Customer, estimate and job workflow"],["Technician","Assigned jobs, inspection and QC"],["Store / Parts","Inventory and suppliers"],["Finance / Accounts","Invoices, payments and reports"]]} />,
    "Notifications": <SettingsCards kicker="FOLLOW-UP PREFERENCES" title="Notification controls" items={[["Service due soon","Human follow-up queue"],["Service overdue","Human follow-up queue"],["Kilometre update","Notify workshop"],["Problem report","Notify service advisor"],["Automatic outbound messaging","Not active in this prototype"]]} />,
  };
  return <><PageHeader eyebrow="WORKSHOP CONFIGURATION" title="Settings" description={`Manage ${WORKSHOP_PROFILE.businessName} without inventing unconfirmed business details.`} actions={<button className="primary-button" onClick={() => notify(`${section} saved in this demo session`)}>Save demo changes</button>} /><div className="settings-layout"><nav className="panel settings-nav">{sections.map((item) => <button className={section===item?"active":""} key={item} onClick={() => setSection(item)}>{item}<span>→</span></button>)}</nav><section className="panel settings-form">{content[section]}</section></div></>;
}

function SettingsCards({ kicker, title, items }: { kicker: string; title: string; items: string[][] }) {
  return <><div className="panel-heading"><div><span className="section-kicker">{kicker}</span><h2>{title}</h2></div></div><div className="settings-detail-list">{items.map(([label,value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></>;
}
...
function OperationalModule({ view, notify }: { view: WorkshopView; notify: (message: string) => void }) {
  const content: Record<string, {eyebrow:string; title:string; description:string; headers:string[]; rows:string[][]; action:string}> = {
    ...
    payments:{eyebrow:"PAYMENTS & RECEIPTS",title:"Payments",description:"Record EFT, card and cash payments against invoices with clear balances.",headers:["Date","Reference","Customer","Invoice","Method","Amount","Recorded by"],rows:[["20 Aug 2026","SIPHO-2008","Sipho Dlamini","JB-INV-0194","EFT","R2,500.00","Lindiwe Maseko"],["20 Aug 2026","CARD-8841","Lerato Molefe","JB-INV-0192","Card","R3,680.00","Lindiwe Maseko"],["19 Aug 2026","CASH-0191","Kabelo Ndlovu","JB-INV-0191","Cash","R1,450.00","Jabulani"]],action:"+ Record payment"},
...
function CustomerPortal({ onExit, reports, onSubmitReport }: { onExit: () => void; reports: ProblemReport[]; onSubmitReport: (report: ProblemReport) => ReportMutationResult }) {
  const [view, setView] = useState<CustomerView>("home");
  return <div className="portal-stage"><aside className="portal-context"><BrandLockup /><span className="section-kicker">CUSTOMER CARE PORTAL</span><h1>Your vehicle, professionally managed from arrival to its next service.</h1><p>View service needs, update kilometres and contact the workshop without navigating workshop complexity.</p><button className="secondary-dark" onClick={onExit}>← Return to workshop platform</button><div className="portal-help"><span>Need help?</span><strong>Contact JB Motors</strong><small>Workshop contact details require owner confirmation.</small></div></aside><main className="customer-app"><header className="customer-header"><BrandLockup compact /><button onClick={onExit}>Workshop view</button></header><div className="customer-content">{view === "home" && <CustomerHome onNavigate={setView} />}{view === "vehicles" && <CustomerVehicle onNavigate={setView} />}{view === "service" && <ServiceRequest onDone={() => setView("home")} />}{view === "problem" && <ProblemReport onDone={() => setView("home")} onSubmit={onSubmitReport} />}{view === "kilometres" && <UpdateKilometres onDone={() => setView("home")} />}{view === "history" && <CustomerHistory reports={reports} />}{view === "profile" && <CustomerProfile />}</div><nav className="customer-bottom-nav" aria-label="Customer portal navigation">{[
...
function ProblemReport({ onDone, onSubmit }: { onDone: () => void; onSubmit: (report: ProblemReport) => ReportMutationResult }) {
  return <ProblemReportForm onDone={onDone} onSubmit={onSubmit} />;
}

function CustomerHistory({ reports }: { reports: ProblemReport[] }) {
  return <><section className="mobile-page-heading no-back"><div><span className="section-kicker">VEHICLE RECORD</span><h1>History and reports</h1><p>Completed work and problem reports saved on this device.</p></div></section>{reports.length > 0 && <section className="customer-report-history"><div className="customer-section-heading"><span className="section-kicker">YOUR PROBLEM REPORTS</span><h2>Submitted to JB Motors</h2></div>{reports.map((report) => <article key={report.id}><div><StatusPill tone={report.status === "Resolved" ? "green" : report.status === "Under review" ? "amber" : "blue"}>{report.status.toUpperCase()}</StatusPill><time>{new Date(report.submittedAt).toLocaleDateString("en-ZA", { dateStyle: "medium" })}</time></div><h3>{report.reference}</h3><p>{report.customer.name} · {report.vehicle.label} · {report.vehicle.registration}</p>{report.issues.map((issue, index) => <section key={issue.id}><h4>Issue {index + 1}: {issue.categories.join(" / ")}</h4><p style={{ whiteSpace: "pre-wrap", overflowWrap: "anywhere" }}>{issue.description}</p><p>{issue.occurrence} · {issue.severity}</p></section>)}<strong>{report.issues.length} {report.issues.length === 1 ? "issue" : "issues"}</strong></article>)}</section>}<div className="mobile-history">{SERVICE_HISTORY_DOCUMENT.entries.map((entry) => <article key={entry.date}><time>{entry.date}</time><StatusPill tone="green">COMPLETED</StatusPill><h2>{entry.service}</h2><p>{entry.kilometres} · Toyota Corolla</p><strong>{entry.total}</strong><button>View details →</button></article>)}</div></>;
}
...
export default function Home() {
  ...
  const [interfaceState, setInterfaceState] = useState<InterfaceState>("ready");
  const [problemReports, setProblemReports] = useState<ProblemReport[]>([]);
  const [reportStorageWarning, setReportStorageWarning] = useState<string | null>(null);
  useEffect(() => {
    const loaded = loadProblemReports();
    setProblemReports(loaded.reports);
    setReportStorageWarning(loaded.warning);
  }, []);
  const visibleNav = navItems.filter((item) => item.roles.includes(role));
  const notify = (message: string) => setToast(message);
  const submitProblemReport = (report: ProblemReport): ReportMutationResult => {
    const next = [report, ...problemReports];
    const result = saveProblemReports(next);
    if (result.ok) {
      setProblemReports(next);
      setReportStorageWarning(null);
    } else setReportStorageWarning(result.message);
    return result;
  };
  const updateProblemReportStatus = (id: string, status: ProblemReportStatus): ReportMutationResult => {
    const next = problemReports.map((report) => report.id === id ? { ...report, status, updatedAt: new Date().toISOString() } : report);
    const result = saveProblemReports(next);
    if (result.ok) {
      setProblemReports(next);
      setReportStorageWarning(null);
  ...
  const resetProblemReports = () => {
    if (!window.confirm("Remove only the problem reports saved by this local demo? This cannot be undone.")) return;
    const result = clearProblemReports();
    if (result.ok) {
      setProblemReports([]);
      setReportStorageWarning(null);
      notify("Saved demo problem reports were reset");
    } else {
      setReportStorageWarning(result.message);
  ...
  const renderView = () => {
    ...
    if (activeView === "requests") return <Requests reports={problemReports} storageWarning={reportStorageWarning} onUpdateStatus={updateProblemReportStatus} onReset={resetProblemReports} notify={notify} />;
    if (activeView === "reports") return <Reports notify={notify} />;
    if (activeView === "settings") return <Settings notify={notify} />;
    if (activeView === "technician") return <TechnicianView notify={notify} />;
    return <OperationalModule view={activeView} notify={notify} />;
  };
  if (portalOpen) return <CustomerPortal onExit={() => setPortalOpen(false)} reports={problemReports} onSubmitReport={submitProblemReport} />;
  return <main className="app-shell"><aside className="sidebar"><BrandLockup /><nav className="side-nav" aria-label="Workshop navigation">{visibleNav.map((item) => <button className={`nav-item ${activeView===item.view?"active":""}`} key={item.view} onClick={() => setActiveView(item.view)}><span className="nav-code">{item.code}</span>{item.label}{item.view==="follow-ups"&&<b className="nav-count">7</b>}</button>)}</nav>{role === "Owner / Administrator" && <button className={`nav-item settings ${activeView==="settings"?"active":""}`} onClick={() => setActiveView("settings")}><span className="nav-code">ST</span>Settings</button>}<div className="user-card"><span className="avatar">J</span><span><strong>{WORKSHOP_PROFILE.ownerName}</strong><small>{role}</small></span></div></aside><section className="workspace"><header className="topbar"><button className="mobile-brand" onClick={() => setActiveView("dashboard")}><BrandLockup compact /></button><button className="mobile-portal-button" onClick={() => setPortalOpen(true)}>Customer portal</button><div className="desktop-greeting"><span className="eyebrow">THURSDAY · 20 AUGUST 2026</span><strong>JB Motors Workshop</strong></div><div className="top-actions"><StatusPill tone="orange">LOCAL UI PROTOTYPE</StatusPill><label className="demo-state-select"><span>STATE</span><select value={interfaceState} onChange={(event) => setInterfaceState(event.target.value as InterfaceState)}><option value="ready">Ready</option><option value="loading">Loading</option><option value="empty">Empty</option><option value="error">Error</option></select></label><label className="role-select"><span>VIEW AS</span><select value={role} onChange={(event) => changeRole(event.target.value as StaffRole)}>{["Owner / Administrator","Service Advisor","Technician","Store / Parts","Finance / Accounts"].map((item) => <option key={item}>{item}</option>)}</select></label><button className="secondary-button portal-button" onClick={() => setPortalOpen(true)}>Customer portal</button><button className="primary-button" onClick={() => {setActiveView("job-card");notify("New draft job card opened");}}>+ New job card</button></div></header><div className="content">{interfaceState === "ready" ? renderView() : <InterfaceStatePanel state={interfaceState} onReset={() => setInterfaceState("ready")} />}</div><nav className="staff-mobile-nav">{visibleNav.slice(0,5).map((item) => <button className={activeView===item.view?"active":""} key={item.view} onClick={() => setActiveView(item.view)}><span>{item.code}</span><small>{item.label}</small></button>)}</nav></section>{toast && <div className="toast" role="status"><span>✓</span><p>{toast}</p><button aria-label="Dismiss notification" onClick={() => setToast("")}>×</button></div>}</main>;
}
